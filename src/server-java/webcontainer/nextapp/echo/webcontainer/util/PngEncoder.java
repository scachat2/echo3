...
/**
 * Encodes a java.awt.Image into PNG format. For more information on the PNG
 * specification, see the W3C PNG page at
 * <a href="http://www.w3.org/TR/REC-png.html">http://www.w3.org/TR/REC-png.html</a>.
 */
public class PngEncoder {

    /**
     * Utility class for converting <code>Image</code>s to
     * <code>BufferedImage</code>s.
     */
    private static class ImageToBufferedImage {

        /**
         * Converts an <code>Image</code> to a <code>BufferedImage</code>. If
         * the image is already a <code>BufferedImage</code>, the original is
         * returned.
         *
    ...
    public interface Filter {
         ...
         * @param filterOutput a byte array into which the filtered data will be
         * placed
         * @param outputBpp number of bit per pixel
         */
    ...
    private static class SubFilter
        ...
        /**
         * @see nextapp.echo.webcontainer.util.PngEncoder.Filter#filter(byte[],
         * byte[], byte[], int)
         */
        public void filter(byte[] filterOutput, byte[] currentRow, byte[] previousRow, int outputBpp) {
    ...
    /**
     * An interface for translators, which translate pixel data from a writable
     * raster into an R/G/B/A ordering required by the PNG specification. Pixel
     * data in the raster might be available in three bytes per pixel, four
     * bytes per pixel, or as integers.
     */
    interface Translator {

        /**
         * Translates a row of the image into a byte array ordered properly for
         * a PNG image.
         *
    ...
    private class ByteTranslator
        ...
        /**
         * @see
         * nextapp.echo.webcontainer.util.PngEncoder.Translator#translate(byte[],
         * int)
         */
        public void translate(byte[] outputPixelQueue, int row) {
     ...
     * for compressing the image data: Valid values range from 0 to 9. Higher
     * values result in smaller files and therefore decrease network traffic,
     * but require more CPU time to encode. The normal compromise value is 3.
     */
     ...
     * Encodes the image.
     *
     * @param out an OutputStream to which the encoded image will be written
     ...
     * @param csum the Checksum that is updated as data is written to the
